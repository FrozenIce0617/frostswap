...
describe("Exchange", () => {
  ...
  it("is deployed", async () => {
    ...
    expect(await exchange.name()).to.equal("Frostswap-V1");
    expect(await exchange.symbol()).to.equal("FROST-V1");
  ...
  describe("removeLiquidity", async () => {
    ...
    it("removes some liquidity", async () => {
      ...
      ).to.equal("24.999935399843232626"); // 25 - gas fees
    ...
    it("removes all liquidity", async () => {
      ...
      expect(
        fromWei(userEtherBalanceAfter.sub(userEtherBalanceBefore))
      ).to.equal("99.999948658291156756"); // 100 - gas fees
    ...
    it("pays for provided liquidity", async () => {
      ...
      expect(
        fromWei(userEtherBalanceAfter.sub(userEtherBalanceBefore))
      ).to.equal("109.999948864349577356"); // 110 - gas fees
